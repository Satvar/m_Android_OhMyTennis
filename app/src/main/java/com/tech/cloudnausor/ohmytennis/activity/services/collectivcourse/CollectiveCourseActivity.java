package com.tech.cloudnausor.ohmytennis.activity.services.collectivcourse;
...
public class CollectiveCourseActivity extends AppCompatActivity {
    ...
    ,price_4_person,price_5_person,price_6_person,user_desc;
    TextView course_address,course_bike,course_train,course_van,course_email,course_contact,course_bus,course_trans;
...
//        postalcodecity = getIntent().getStringExtra("postal_code_city");
        ...
        course_address=(TextView)findViewById(R.id.course_address);
        course_bike=(TextView)findViewById(R.id.course_bike);
        course_trans=(TextView)findViewById(R.id.course_trans);
        course_bus=(TextView)findViewById(R.id.course_bus);
        course_train=(TextView)findViewById(R.id.course_train);
        course_van=(TextView)findViewById(R.id.course_van);
        course_contact=(TextView)findViewById(R.id.course_contact);
...
//                Uri gmmIntentUri = Uri.parse("geo:0,0?q="+redirect_address);
        ...
        User_coach_reserce.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(CollectiveCourseActivity.this, DefaultCalendarActivity.class);
...
//                intent.putExtra("pl_five",onDeamndModel.getPrice_6pl_1hr());
    ...
    public void getOnDemand(){
        singleTonProcess.show(CollectiveCourseActivity.this);
                    ...
                    if(response.body().getIsSuccess().equals("true")) {
                        if (!response.body().getGetIndiCoachDetailsModel().getOndemandCourseModels().isEmpty()){
                            if (response.body().getGetIndiCoachDetailsModel().getOndemandCourseModels().get(0) != null) {
                                ...
                                System.out.println(" onDeamndModel----> "+ new Gson().toJson(onDeamndModel) );
                                ...
                                user_desc.setText(onDeamndModel.getDescription());
                                course_address.setText(onDeamndModel.getLocation());
                                course_trans.setText(onDeamndModel.getMode_of_transport());
                                course_email.setText(gmail);
                                course_contact.setText(phone);

//                                Car,Vélo,Train,Bus
                                redirect_address = onDeamndModel.getLocation()+" "+onDeamndModel.getPostalcode();
